...
if __name__ == "__main__":
    ...
    switch_initial_state = args.Switch_Initial_State
    ...
    if long_score != 'no_use':
        long_score = float(long_score)
    short_score = args.Short_Score
    if short_score != 'no_use':
    ...
    use_FC = args.use_FC
    temp_loss = 1000000000000000 # Prevent the loss from rising for the first time
    
    # Give name to log_file_name
    if fusion_way == 'none':
        log_file_name = (f"LCII_ver_{strategy}_dropout_{dropout_pkeep}")
    else:
        if fusion_way == 'fix': 
            log_file_name = (f"LCII_ver_{strategy}_w-{window}_{fusion_way}_L-{long_score}_S-{short_score}_dropout_{dropout_pkeep}")
        else:
            log_file_name = (f"LCII_ver_{strategy}_w-{window}_{fusion_way}_dropout_{dropout_pkeep}")
    if use_FC == True:
        log_file_name = (f"{log_file_name}_FC")
    ...
    if dataset == "Amazon":
        ...
        #dataset_path = './datasets/'+dataset+'/pickle/4_train_test_split_sapmle.pickle'
    ...
    if strategy == 'original':
        ...
        if int((SEQLEN+1) * window * 0.01) < 1:
            capture_len_h_list = -1
        else:
            capture_len_h_list = int((SEQLEN+1) * -window * 0.01)
    ...
    if use_FC == True:
        input_sum = layers.linear(input_sum, EMBEDDING_SIZE) # Whether to finally add FC to input_sum
    ...
    while epoch <= MAX_EPOCHS:
        ...
        if do_training:
            ...
            while len(xinput) > int(BATCHSIZE/2):
                ...
                if _batch_number == 1:
                    print(f"Train | Batch number: {_batch_number} / {num_training_batches}")
        ...
        if Threshold < lgr and epoch > 1: # recall@5 > Threshold and epoch > 1
