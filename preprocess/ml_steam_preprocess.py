...
def check_dataset_path(dataset):
    ...
    if dataset == MovieLens_1M:
        ...
        if not isExists_file:
            os.makedirs(DATASET_DIR + '/steam')
        
        # Move file
        if not file_exists(DATASET_DIR + '/steam/steam_new.json'):
            shutil.move(DATASET_DIR + '/steam_new.json', DATASET_DIR + '/steam')
    ...
    if not isExists_file:
        os.makedirs(DATASET_DIR + '/pickle') 
    ...
    with open(DATASET_FILE, 'rt', buffering=10000, encoding='utf8') as datasets:
        for line in datasets:
            if dataset == MovieLens_1M:
                line = line.rstrip()
                line = line.strip("\n") 
                line = line.split(",") # Separate strings with commas
                user_id   = line[0]
                item      = line[1]
...
def sort_and_split_usersessions():
    ...
    with open(os.path.join(DATASET_DIR + "/pickle", 'uid2uindex.pkl'), 'wb') as fp:
    ...
    with open(os.path.join(DATASET_DIR + "/pickle", 'iid2iindex.pkl'), 'wb') as fp:
...
if __name__ == "__main__":
    ...
    if not file_exists(DATASET_FILE):
